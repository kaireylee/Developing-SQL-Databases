CREATE TABLE [dbo].[Customers] (
    ...
    CONSTRAINT [PK_Customers] PRIMARY KEY CLUSTERED ([CustomerID] ASC) WITH (DATA_COMPRESSION = PAGE)
);
...
GO
CREATE NONCLUSTERED INDEX [AddressRegionPostalCode]
    ON [dbo].[Customers]([PostalCode] ASC)
    INCLUDE([Address], [City], [Region]);


GO
CREATE NONCLUSTERED INDEX [CustomerRegion]
    ON [dbo].[Customers]([Region] ASC) WHERE ([Region]='OR');
